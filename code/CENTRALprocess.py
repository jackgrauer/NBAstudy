...
def process_CENTRAL():
        ...
        for city_code, city_info in CENTRAL['Cities'].items():
            for key in ['wagetax', 'salestax', 'NBArev']:
                if key in city_info:
                    ...
                    adjusted_values = []
                    for year, value in zip(CENTRAL['YEARS'], original_values):
    ...
    for i in range(len(years)):
        nba_rev_total = sum(CENTRAL['Cities'][city]['NBArev'][i] for city in cities_to_aggregate if 'NBArev' in CENTRAL['Cities'][city] and CENTRAL['Cities'][city]['NBArev'][i] is not None)
        sales_tax_total = sum(CENTRAL['Cities'][city]['salestax'][i] for city in cities_to_aggregate if 'salestax' in CENTRAL['Cities'][city] and CENTRAL['Cities'][city]['salestax'][i] is not None)
        wage_tax_total = sum(CENTRAL['Cities'][city]['wagetax'][i] for city in cities_to_aggregate if 'wagetax' in CENTRAL['Cities'][city] and CENTRAL['Cities'][city]['wagetax'][i] is not None)
    ...
    # Step 19: Stationary Bootstrapping
    import numpy as np
